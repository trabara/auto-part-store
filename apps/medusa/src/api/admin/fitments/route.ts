...
export async function POST(
  ...
  const fitment = await fitmentModuleService.createFullFitment(req.validatedBody)
  res.json({ fitment })
}

export async function PATCH(req: MedusaRequest<UpdateFitmentInput>, res: MedusaResponse) {
  const fitmentModuleService = req.scope.resolve(FITMENT_MODULE)
  const updated = await fitmentModuleService.updateFitments(req.validatedBody)
