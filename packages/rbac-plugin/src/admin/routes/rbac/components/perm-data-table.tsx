...
const PermRowSchema = PermissionSchema.omit({ category: true, created_at: true, updated_at: true, deleted_at: true })
type PermRow = z.infer<typeof PermRowSchema>

const fetchPermissions = async (signal: AbortSignal, params?: PageQueryParams) => {
...
function PermissionDataTable({ defaultValues, onChange, ...props }: PermissionDataTableProps) {
    const handleChange = useCallback((row: PermRow[]) => {
