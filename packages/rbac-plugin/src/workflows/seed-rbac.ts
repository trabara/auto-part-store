...
const seedRbacStep = createStep(
  ...
  async (_, { container }) => {
    ...
    const [existingCategories] = await service.listAndCountAuthzCategories({});
    ...
    for (const cat of DEFAULT_CATEGORIES) {
      const created = await service.createAuthzCategories(cat);
    ...
    for (const perm of PREDEFINED_PERMISSIONS) {
      ...
      const created = await service.createAuthzPermissions({
  ...
  async (compensation, { container }) => {
    ...
    if (compensation.permissionIds?.length) {
      await service.deleteAuthzPermissions(compensation.permissionIds);
    }
    if (compensation.categoryIds?.length) {
      await service.deleteAuthzCategories(compensation.categoryIds);
    }
  },
);
