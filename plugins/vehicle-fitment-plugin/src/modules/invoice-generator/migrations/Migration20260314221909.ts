...
export class Migration20260314221909 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "invoice" ("id" text not null, "display_id" serial, "order_id" text not null, "status" text check ("status" in ('latest', 'stale')) not null default 'latest', "content" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "invoice_pkey" primary key ("id"));`);
